...
find "$VENDOR_DIR" -name "*.go" -type f -exec sed -i '' \
...
sed -i '' 's|^module github.com/microsoft/typescript-go|module goodchanges/tsgo-vendor|' "$VENDOR_DIR/go.mod"
...
sed -i '' 's|github.com/microsoft/typescript-go|goodchanges/tsgo-vendor|g' "$VENDOR_DIR/go.mod"
